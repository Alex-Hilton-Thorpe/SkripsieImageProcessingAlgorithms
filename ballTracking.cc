...
#include <fstream> //Reading from textfiles
#include <sstream>
...
int main(int, char *argv[])
{
  ...
  try
  {
    ...
    std::vector<double> posAlg1x, posAlg1y, posAlg2x, posAlg2y, posAlg3x, posAlg3y; // Creating arrays (vectors) from all the x and y positions
    ...
    cov_Ipa(0, 0) = cov_Ipa(1, 1) = 100.0;   // sets the confidence I have in the algorihtms, lower the more confident
    ...
    for (int d = 0; d < 4; d++)
    {
      ...
      cov_trans(d, d) = 15.0;  // set diagonal // sets the confience I have in the model, lower the more confident
    ...
    for (int i = 0; i < frames; i++)
    {
      ...
      if (!ignoreAlg3)
      {
        rcptr<Factor> alg_3(uniqptr<SG>(new SG({x, y}, mn3, cov_Ipa))); // Setting up the Guassian factor for IPA3
